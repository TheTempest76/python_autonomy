...
def arm_and_takeoff(master, altitude):
    ...
    while True:
        ...
        print(f"[TAKEOFF] Altitude: {current_alt:.2f} m")
...
def get_current_position(master):
    """Get current GPS position and print it."""
    msg = master.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
    if msg is None:
        return 0.0, 0.0, 0.0
...
def goto_waypoint(master, lat, lon, alt, acceptance_radius=2.0):
    ...
    while True:
        ...
        print(f"[NAV] Distance to waypoint: {distance:.2f} m, "
...
def main():
    parser = argparse.ArgumentParser(
        description='Fly drone in lawnmower pattern'
    )
    parser.add_argument(
        '--connection',
    ...
    parser.add_argument(
        '--kml',
        type=str,
        help='Path to KML file with area polygon'
    )
    parser.add_argument(
        '--altitude',
        type=float,
